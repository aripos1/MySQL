...
select department_id 부서
		,avg(salary) 평균임금
        ,min(salary) 최저임금
        ,avg(salary)-min(salary) '평균입금 - 최저임금'
from employees
group by department_id
...
select first_name 이름
		...
		,case when hire_date <20021231 then '창립맴버'
			  when hire_date <20031231 then '03년입사'
			  when hire_date <20041231 then '04년입사'
              else '상장이후입사'
		  end optDate
from employees
...
select case dayofweek(min(hire_date))
	    when '7' then date_format(min(hire_date), '%Y년 %m월 %d일 (토요일)') 
		end 입사일
from employees
;

select SUBSTR('일월화수목금토', DAYOFWEEK(min(hire_date)), 7) as 토
from employees ;
